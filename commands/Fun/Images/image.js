    ...
    async run(msg, [...params]) {
        ...
        async function result(error, results) {
            if (error) {
                ...
                if (isProfanity) msg.channel.send("|| " + results[Math.floor(Math.random() * results.length)].url + " ||")
                else msg.channel.send(results[Math.floor(Math.random() * results.length)].url)
            }
        }
